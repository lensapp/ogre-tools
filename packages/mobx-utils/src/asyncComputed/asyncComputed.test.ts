import { IAsyncComputed, asyncComputed } from './asyncComputed';
...
describe('asyncComputed', () => {
  describe('given callback to observe something that returns a promise, and a specific value for when pending', () => {
    let someMock: AsyncFnMock<(value: string) => Promise<unknown>>;
    let someAsyncComputed: IAsyncComputed<unknown, undefined>;
  ...
  describe('given callback that accepts an AbortSignal to observe something that returns a promise, and a specific value for when pending', () => {
    let someMock: AsyncFnMock<
      (value: string, signal: AbortSignal) => Promise<unknown>
    >;
    let someAsyncComputed: IAsyncComputed<unknown, undefined>;
    let someObservable: IObservableValue<string>;
    ...
    beforeEach(() => {
      ...
      someAsyncComputed = asyncComputed({
        getValueFromObservedPromise: signal => {
          const someObservedValue = someObservable.get();

          return someMock(someObservedValue, signal);
        },
    ...
    describe('when only status is observed but not value', () => {
      ...
      it('when status is observed, computes', () => {
        expect(someMock).toHaveBeenCalledWith(
          'some-initial-value',
      ...
      it('should not have an aborted signal', () => {
        expect(someMock.mock.calls[0][1].aborted).toBe(false);
      });
    ...
    describe('given value is observed', () => {
      ...
      describe('when observed promise has not resolved yet', () => {
        ...
        describe('but another change is observed', () => {
          ...
          it('aborts the signal passed to the first call', () => {
            expect(someMock.mock.calls[0][1].aborted).toBe(true);
          });
